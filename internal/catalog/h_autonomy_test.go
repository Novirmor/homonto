	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			[]string{
				...
				"under configured permissions without individual approval",
				"Inspect commands and package scripts for relevance",
				"allowed or auto-approved runs are valid evidence",
				...
				"mutations retain their tool permission prompt boundary",
				"read-only context fetches need no redundant user dialog when the tool is allowed",
