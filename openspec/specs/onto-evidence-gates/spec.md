# onto-evidence-gates Specification

## Purpose
The onto binary is the authority on whether a change may leave a phase. Where
a decision needs human judgment — the proposal is sound, the approach is
agreed, the close may rewrite shared specs — the binary cannot judge, so it
requires recorded evidence that the gate was answered, and refuses the
transition until the token is there. The same principle covers the claims a
change makes about itself: a recorded verify pass is cross-checked against the
report, and the working phase is derived from workspace artifacts rather than
trusted from the state file. The target is the honest agent that would
otherwise skip a step, not the dishonest one that fabricates evidence.
